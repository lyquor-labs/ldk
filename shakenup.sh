...
setup_rustup() {
    ...
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- --default-toolchain 1.96.0 --profile minimal --no-modify-path -y
...
setup_rust_toolchain() {
    ...
    if ! rustup toolchain list | grep -q "^1.96.0"; then
        log_step "Installing Rust 1.96.0 toolchain..."
        rustup toolchain install 1.96.0
        needs_setup=true
    fi

    rustup default 1.96.0 >/dev/null 2>&1 || {
        log_error "Failed to set Rust 1.96.0 as default toolchain"
        exit 1
    }

    if $needs_setup; then
        log_success "Rust 1.96.0 toolchain configured"
    else
        log_success "Rust 1.96.0 toolchain ready"
    fi
}
...
setup_wasm_target() {
    ...
    # Check wasm32 target
    if ! rustup target list --installed --toolchain 1.96.0 2>/dev/null | grep -q "^wasm32-unknown-unknown"; then
...
rm -rf lyquor_data && LYQUOR_LOG=${LYQUOR_LOG:-info} "$SCRIPT_DIR/lyquor" &
pid=$!
...
command -v rustc >/dev/null && {
    echo "✓ Rust: \$(rustc +1.96.0 --version 2>/dev/null || rustc --version)"
    if rustup target list --installed --toolchain 1.96.0 2>/dev/null | grep -q "^wasm32-unknown-unknown"; then
