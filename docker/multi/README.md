...
Note: current `setup-devnet` service bootstraps bartender by running `shaker deploy --is-bartender --reference ghcr.io/lyquor-labs/lyquids:bartender-v0.4.1 --endpoint ws://node1:10087/ws`, with an idempotency check to skip redeploy when already present.
