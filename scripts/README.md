...
`ghcr.io/lyquor-labs/lyquids`. By default the `bartender-v0.5.0` tag is used.

```bash
# Pin to the release matching this LDK checkout. `v0.5.0` is replaced with
# the release tag when this README is mirrored to a versioned LDK release.
LYQUOR_IMAGE_TAG=v0.5.0 ./localnet.sh single

# Or specify the full reference.
LYQUOR_BARTENDER_REFERENCE=ghcr.io/lyquor-labs/lyquids:bartender-v0.5.0 ./localnet.sh multi
```
...
| Variable                     | Purpose                                                        |
| ---------------------------- | ------------------------------------------------------------- |
| `LYQUOR_IMAGE_TAG`           | Tag used to build the bartender reference (`v0.5.0`).     |
